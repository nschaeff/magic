...
!------------------------------------------------------------------------------
   subroutine updateXi(xi,dxi,dVXirLM,dxidt,dxidtLast,w1,coex,dt,nLMB)
      ...
      !$OMP shared(map_glbl_st,lm2l,lm2m,beta,or1,dLh,or2)
      !$OMP DO
      do iThread=0,nThreads-1
         start_lm=lmStart+iThread*per_thread
         stop_lm = start_lm+per_thread-1
      ...
      do nR=n_r_cmb+1,n_r_icb-1
         do lm1=lmStart,lmStop
            dxidtLast(lm1,nR)=dxidt(lm1,nR) &
                 & - coex*osc*hdif_Xi(map_glbl_st%lm2(lm2l(lm1),lm2m(lm1))) * &
                 &   ( work_LMloc(lm1,nR) &
                 &     + ( beta(nR)+two*or1(nR) ) * dxi(lm1,nR) &
                 &     - dLh(map_glbl_st%lm2(lm2l(lm1),lm2m(lm1)))*or2(nR)*xi(lm1,nR) &
                 &   )
         end do
      end do
      !$OMP end do
