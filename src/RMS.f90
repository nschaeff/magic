...
!-----------------------------------------------------------------------------
   subroutine dtVrms(time,nRMS_sets,timePassed,timeNorm)
      ...
      call MPI_AllgatherV(MPI_IN_PLACE,sendcount,MPI_DEF_REAL,&
           & ArcMag2hInt,recvcounts,displs,MPI_DEF_REAL,comm_r,ierr)
      call MPI_AllgatherV(MPI_IN_PLACE,sendcount,MPI_DEF_REAL,&
      ...
      if ( coord_r == 0 ) then
         ...
         if (rank == 0) write(n_dtvrms_file,'(1P,ES20.12,7ES16.8,7ES14.6)')&
         ...
         &    status='unknown')
         do l=0,l_max
            if (rank == 0) write(fileHandle,'(1P,I4,28ES16.8)') l+1,         &
